...
export function Details( { addToCart, history, products, match } ) {
	...
	return (
		<div className="details">
			...
			<img
				alt={ name /* products name */ }
				className="details__logo"
				src={ logo /* products logo */ }
			/>
		<h1 className="details__name">{ name/* products name*/ }</h1>
			<p className="details__description">{ description/* products description*/ }</p>
			<button
				className="details__buy"
				onClick={ addToCartAndRedirect }
			>
				Buy now for ${ price/* products price */ }!
			</button>
		</div>
	);
}
